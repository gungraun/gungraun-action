...
jest.mock("../detect", () => ({
    ...
    detectPlatform: jest.fn().mockReturnValue({ id: "ubuntu", versionId: "22.04", platform: "ubuntu-22.04", packageManager: "apt-get" }),
...
jest.mock("../resolve", () => ({
    ...
    resolveValgrindAssetName: jest.fn().mockResolvedValue("valgrind-3.20.0-x86_64-ubuntu-22.04.tar.gz"),
...
jest.mock("../utils", () => ({
    ...
    printErr: jest.fn((msg: string) => { throw new Error(msg); }),
...
describe("installGrWithBinstall", () => {
    ...
    it("calls cargo binstall with formatted version", async () => {
        ...
        expect(mockExec).toHaveBeenCalledWith(
            getCargoBin(),
            ["binstall", "-y", "gungraun-runner@1.0.0"]
        );
    });
...
describe("installGrFromSource", () => {
    ...
    it("calls cargo install with formatted version", async () => {
        ...
        expect(mockExec).toHaveBeenCalledWith(
            getCargoBin(),
            ["install", "gungraun-runner", "--version", "1.0.0"]
        );
    });
...
describe("installValgrindWithPackageManager", () => {
    ...
    it("installs with apt-get on ubuntu", async () => {
        mockDetectPlatform.mockReturnValue({ id: "ubuntu", versionId: "22.04", platform: "ubuntu-22.04", packageManager: "apt-get" });

        await installValgrindWithPackageManager();

        expect(mockExec).toHaveBeenCalledWith("sudo", ["apt-get", "update", "-qq"]);
        expect(mockExec).toHaveBeenCalledWith("sudo", ["apt-get", "install", "-y", "-qq", "valgrind"]);
    });

    it("installs with dnf on fedora", async () => {
        mockDetectPlatform.mockReturnValue({ id: "fedora", versionId: "39", platform: "fedora-39", packageManager: "dnf" });
    ...
    it("tries yum then falls back to dnf on amzn", async () => {
        mockDetectPlatform.mockReturnValue({ id: "amzn", versionId: "2", platform: "amzn-2", packageManager: "yum" });
    ...
    it("installs with pacman on arch", async () => {
        mockDetectPlatform.mockReturnValue({ id: "arch", versionId: "rolling", platform: "arch-rolling", packageManager: "pacman" });
    ...
    it("installs with zypper on opensuse", async () => {
        mockDetectPlatform.mockReturnValue({ id: "opensuse-leap", versionId: "15.5", platform: "opensuse-leap-15.5", packageManager: "zypper" });

        await installValgrindWithPackageManager();

        expect(mockExec).toHaveBeenCalledWith("sudo", ["zypper", "--non-interactive", "install", "valgrind"]);
    });

    it("installs with apk on alpine", async () => {
        mockDetectPlatform.mockReturnValue({ id: "alpine", versionId: "3.19", platform: "alpine-3.19", packageManager: "apk" });
    ...
    it("throws on unknown package manager", async () => {
        mockDetectPlatform.mockReturnValue({ id: "gentoo", versionId: "2.14", platform: "gentoo-2.14", packageManager: null });
